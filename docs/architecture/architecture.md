...
Buddy 是一个基于 AI 的实时音视频对话系统，结合了语音识别（ASR）、大语言模型（LLM）和文本转语音（TTS）技术，实现了自然的人机交互体验。项目支持多种 OAuth 登录方式，并提供了完整的前端界面和后端服务架构。
...
Buddy 是一个架构完整、功能强大的 AI 音视频对话系统。它采用了现代的技术栈，结合了多个云服务提供商的 API，提供了自然的人机交互体验。项目代码结构清晰，模块职责明确，易于扩展和维护。
